  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(`${API}/order/${orderId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ status: method === "COD" ? "pending" : "confirmed" }),
      });
              ...
              <div className="card-body p-4">
                ...
                <div className="d-flex gap-3 align-items-center mb-4 p-3 bg-light rounded-3">
                  <img
                    ...
                    onError={e => { (e.target as HTMLImageElement).src = "https://placehold.co/80x80/e8f5e9/198754?text=🌾"; }}
                  />
                  <div>
                ...
                <div className="d-flex justify-content-between align-items-center p-3 bg-success bg-opacity-10 rounded-3">
                  <div>
                    <small className="text-muted">₹{product.price} × {quantity} {product.unit}</small>
              ...
              <div className="card-body p-4">
                ...
                <div className="d-flex gap-2 mb-4 flex-wrap">
                  {[
                    ...
                    <button key={key}
                      ...
                      className={`btn btn-sm rounded-pill px-3 ${method === key ? "btn-success" : "btn-outline-secondary"}`}>
                      {icon} {key}
                    </button>
                  ))}
                </div>
                ...
                {method === "UPI" && (
                  <div>
                    ...
                    <input className="form-control rounded-3 mb-2" placeholder="yourname@upi"
                      value={upiId} onChange={e => setUpiId(e.target.value)} />
                    <div className="d-flex gap-2 flex-wrap mt-1">
                      {["GPay", "PhonePe", "Paytm", "BHIM"].map(app => (
                        <span key={app} className="badge bg-light text-dark border px-3 py-2"
                          style={{ cursor: "pointer", fontSize: 13 }}>{app}</span>
                      ))}
                    </div>
                    <small className="text-muted d-block mt-2">💡 Demo mode — koi real payment nahi hogi</small>
                  </div>
                )}
                ...
                {method === "Card" && (
                  <div className="row g-3">
                    <div className="col-12">
                      ...
                      <input className="form-control rounded-3" placeholder="1234 5678 9012 3456"
                    ...
                    <div className="col-12">
                      ...
                      <input className="form-control rounded-3" placeholder="Anurag Kumar"
                        value={cardName} onChange={e => setCardName(e.target.value)} />
                    </div>
                    <div className="col-6">
                      <label className="form-label fw-semibold">Expiry</label>
                      <input className="form-control rounded-3" placeholder="MM/YY"
                        maxLength={5} value={cardExpiry} onChange={e => setCardExpiry(e.target.value)} />
                    </div>
                    <div className="col-6">
                ...
                {method === "NetBanking" && (
                  <div>
                    ...
                    <select className="form-select rounded-3" value={bank} onChange={e => setBank(e.target.value)}>
                ...
                {method === "COD" && (
                  <div className="alert alert-warning rounded-3 mb-0">
                    ...
                    <p className="mb-0 small">Delivery ke time ₹{totalPrice} cash mein pay karna hoga.</p>
                  </div>
                )}
                ...
                <div className="d-flex gap-2 mt-4">
                  <button className="btn btn-outline-secondary flex-fill rounded-3"
                    onClick={() => router.back()} disabled={loading}>← Back</button>
