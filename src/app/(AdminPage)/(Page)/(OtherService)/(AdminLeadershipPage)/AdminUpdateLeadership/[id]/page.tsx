...
function AdminUpdateLeadership() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/leadership?id=${params.id}`);
        if (res.ok) {
  ...
  const updateData = async (e: FormEvent<HTMLFormElement>) => {
    ...
    let errors: any = {};
    if (!data.name) errors.name = "leadership Name is required";
              ...
              <div className="border p-3 mb-3 bg-gradient">
                <div className="text-center">
                  <h5 className="mb-0 font-weight-medium fw-bold text-white">
                    {" "}
                    Update Leadership Service
                    <Link href="/AdminLeadership" className="ms-2">
...
export default AdminUpdateLeadership;
