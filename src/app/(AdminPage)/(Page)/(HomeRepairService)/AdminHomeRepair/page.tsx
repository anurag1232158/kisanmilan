...
function AdminHome() {
  ...
  useEffect(() => {
    fetch("/api/services")
      ...
      .then((data) => {
        ...
        setAboutUsData(data.homeRepair || data);
      })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/homeRepair?id=${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
                ...
                <div className="col-md-12 col-sm-12 m-auto">
                  <div className="card">
                    <div className="card-body bg-grays1">
                      ...
                      >
                        Home Repair Services : {aboutUsData.length}
                        <Link href="/AdminAddHomeRepair" className="mx-2">
                            ...
                            {currentContent.length > 0 ? (
                              currentContent.map((item, index) => (
                                <tr key={item.id} className="text-center">
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminViewHomeRepair/${item.id}`}
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminUpdateHomeRepair/${item.id}`}
