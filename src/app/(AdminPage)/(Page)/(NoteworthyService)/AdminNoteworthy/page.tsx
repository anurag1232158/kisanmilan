...
function AdminHome() {
  ...
  useEffect(() => {
    fetch("/api/services")
      .then((res) => res.json())
      .then((data) => {
        // If your data.json has a key like "noteworthy", use that:
        setAboutUsData(data.noteworthy || data);
      })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/noteworthy?id=${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
                ...
                <div className="col-md-12 col-sm-12 m-auto">
                  <div className="card">
                    <div className="card-body bg-grays1">
                      ...
                      >
                        Noteworthy Services : {aboutUsData.length}
                        <Link href="/AdminAddNoteworthy" className="mx-2">
                            ...
                            {currentContent.length > 0 ? (
                              currentContent.map((item, index) => (
                                <tr key={item.id} className="text-center">
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminViewNoteworthy/${item.id}`}
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminUpdateNoteworthy/${item.id}`}
