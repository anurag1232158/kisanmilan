...
function AdminHome() {
  ...
  useEffect(() => {
    fetch("/api/services")
      ...
      .then((data) => {
        ...
        setAboutUsData(data.painting || data);
      })
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/painting?id=${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
                ...
                <div className="col-md-12 col-sm-12 m-auto">
                  <div className="card">
                    <div className="card-body bg-grays1">
                      ...
                      >
                        Painting Services : {aboutUsData.length}
                        <Link href="/AdminAddPainting" className="mx-2">
                            ...
                            {currentContent.length > 0 ? (
                              currentContent.map((item, index) => (
                                <tr key={item.id} className="text-center">
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminViewPainting/${item.id}`}
                                  ...
                                  <td>
                                    <Link
                                      href={`/AdminUpdatePainting/${item.id}`}
