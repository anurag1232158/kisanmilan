...
function AdminHome() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/mostbooked");
        if (response.ok) {
  ...
  const postData = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/mostbooked", {
        method: "POST",
      ...
      if (res.ok) {
        alert("✅ Most booking Service added successfully!");
            ...
            <div className="container-fluid px-4">
              <div className="border">
                <div className="text-center">
                  ...
                  >
                    Add Most booking Service
                    <Link href="/AdminMostbooked">
                      {" "}
