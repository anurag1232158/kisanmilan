...
function AdminUpdateAppRepair() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/applianceRepair?id=${params.id}`);
        if (res.ok) {
          const result = await res.json();
          setData(result);
        }
      } catch (err) {
        console.error("Error fetching AppRepair:", err);
      }
    };
  ...
  const updateData = async (e: FormEvent<HTMLFormElement>) => {
    ...
    let errors: any = {};
    if (!data.name) errors.name = "AppRepair Name is required";
              ...
              <div className="border p-3 mb-3 bg-gradient">
                <div className="text-center">
                  <h5 className="mb-0 font-weight-medium fw-bold text-white">
                    {" "}
                    Update Appliance Repair Service
                    <Link href="/AdminAppRepair" className="ms-2">
...
export default AdminUpdateAppRepair;
